...
export const errorLog: MessageBuilder = {
	...
	async build(
		...
		const title = new discord.TextDisplayBuilder().setContent(
			`# ${client.getTranslation('error_log.title', locale)}`
		);
		const meta = new discord.TextDisplayBuilder().setContent(
			`**${client.getTranslation('error_log.id', locale)}:** \`${errorId}\`\n` +
				`**${client.getTranslation('error_log.command', locale)}:** \`${context.command || 'unknown'}${context.subcommand ? '.' + context.subcommand : ''}\`\n` +
				`**${client.getTranslation('error_log.user', locale)}:** <@${context.user || 'unknown'}>\n` +
				`**${client.getTranslation('error_log.guild', locale)}:** ${context.guild ? context.guild : 'DM'}\n` +
				`**${client.getTranslation('error_log.created', locale)}:** <t:${Math.floor(now.getTime() / 1000)}:F>`
		);
		...
		const errorMessage = new discord.TextDisplayBuilder().setContent(
			`**${client.getTranslation('error_log.message', locale)}**\n\u200B\n\`\`\`${error.message}\`\`\``
		);
		const stackTrace = new discord.TextDisplayBuilder().setContent(
			`**${client.getTranslation('error_log.stack_trace', locale)}**\n\u200B\n\`\`\`${error.stack ? error.stack.slice(0, 1500) : ''}${error.stack && error.stack.length > 1500 ? '\n...truncated' : ''}\`\`\``
		);

		const developerInfo = new discord.TextDisplayBuilder().setContent(
			`**${client.getTranslation('error_log.developer_info', locale)}:** \`${errorId}\`\n**${client.getTranslation('error_log.time', locale)}:** ${now.toISOString().slice(0, 13).replace('T', ' ')}h UTC`
		);
