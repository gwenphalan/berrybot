	...
	async build(
		...
		switch (action) {
			...
			case 'edit':
				...
				if (!c)
					return menu.setDisabled(true).setPlaceholder('This category has been deleted');

				menu.setOptions(
					guildRoles.map((r) => ({
						label: r.name,
						value: r.id,
						default: c.roles.includes(r.id),
					}))
				)
					.setMinValues(1)
					.setMaxValues(guildRoles.size);
			...
			case 'assign':
				...
				if (!database || !database.selfRoles || !_category)
					return menu.setDisabled(true).setPlaceholder('No roles to select');
	...
	async execute(
		...
		switch (data.action) {
			case 'create':
				...
				else if (!database.selfRoles.categories)
					database.selfRoles.categories = [create_category];
			...
			case 'edit':
				...
				if (database.selfRoles && database.selfRoles.categories)
					database.selfRoles.categories = database.selfRoles.categories.map((c) =>
						c.name === data.category ? edit_category : c
					);
			...
			case 'assign':
				...
				const assign_category = database.selfRoles?.categories?.find(
					(c) => c.name === data.category
				);
